...
def get_jv_data(filedata):
    ...
    if file_type == 'labview':
        ...
        voc_help = df_curves.iloc[:, 0][np.where(np.diff(np.signbit(df_curves.iloc[:, 1])))[0].item()]
        jsc_help = df_curves.iloc[:, 1][np.where(np.diff(np.signbit(df_curves.iloc[:, 0])))[0].item()]
